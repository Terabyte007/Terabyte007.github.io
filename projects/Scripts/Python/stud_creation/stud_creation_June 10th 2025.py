...
# Check for programme column
programme_column_exists = "programme" in df.columns
...
# === 5. LOAD PROGRAMMES, FACULTY, DEPARTMENT, AND COURSE MAPPINGS ===
print("=== LOADING PROGRAMMES, FACULTY, DEPARTMENT, AND COURSE MAPPINGS ===")
# Build programme_map if the 'programme' column exists in Excel
if programme_column_exists:
    cursor.execute("SELECT id, description FROM tbl_programmes")
    programme_map = {desc.strip().lower(): pid for pid, desc in cursor.fetchall()}
else:
    programme_map = {}
...
# === 6. INSERT ANY MISSING COURSES INTO tbl_courses_test ===
print("=== INSERTING MISSING COURSES INTO tbl_courses_test (IF ANY) ===")
...
for _, row in df.iterrows():
    ...
    if normalized_code not in course_code_map:
        ...
        course_unit = abs(int(row.get("course_unit") or 0))
        ...
        insert_course_sql = """
            INSERT INTO tbl_courses_test (title, code, unit, dept_id_fk, lev_id_fk, last_mod_by, last_mod_ts)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        cursor.execute(insert_course_sql, (
            course_title, course_code_raw, course_unit, dept_id, level_id_fk, '654321', current_time
        ))
        connection.commit()
...
# === 7. CREATE students_courses TABLE IF NOT EXISTS ===
print("=== CREATING students_courses TABLE IF NOT EXISTS ===")
cursor.execute(f"""
...
# === 8. POPULATE students_courses, tbl_students_master_test, AND SYNC DEPARTMENT CHANGES ===
print("=== POPULATING students_courses, tbl_students_master_test, AND SYNCING DEPARTMENT CHANGES ===")
...
for _, row in tqdm(df.iterrows(), total=len(df), desc="  Populating students_courses & tbl_students_master_test:"):
    ...
    if programme_column_exists:
        prog_val = row.get("programme")
        if prog_val is not None:
            prog_val_norm = str(prog_val).strip().lower()
            programme_id = programme_map.get(prog_val_norm, 0)
        else:
            programme_id = 0
    else:
        programme_id = 0
    ...
    # Insert into students_courses for reference
    data_tuple = (
    ...
    # -- MAIN LOGIC: Insert or update tbl_students_master_test
    ...
    if not master_row:
        ...
        insert_student_query = """
            INSERT INTO tbl_students_master_test (
                ...
                mode_id, is_new, is_new_level, is_AFM, previous_dept, point_system, prev_session_admitted_id_fk_ch, remark
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s,
                %s, %s, %s, %s,
                %s, %s, %s, %s, %s, %s, %s, %s
        ...
        """
        student_data = (
            matric_no, surname, firstname, middlename, gender, '0000-00-00', 'Nigerian',
            result_session_id, faculty_id, departments_id, 0,
            mode_id, '', '', '', '', '5', '', 'freshers from excel'
        )
        ...
        if db_dept_id != departments_id and departments_id is not None:
            ...
            cursor.execute("""
                ...
                SET 
                    ...
                    department_id_fk = %s,
                    session_admitted_id_fk = %s,
                    point_system = %s,
                    remark = %s
                WHERE matricNo = %s
            """, (db_dept_id, db_session_id, departments_id, result_session_id, '5', 'dept_change', matric_no))
...
# === 9. EXPORT DEPARTMENT CHANGES LOG TO CSV FOR AUDIT ===
if dept_changes_log:
...
# === 10. POPULATE tbl_students_transactions_test TABLE ===
print("=== POPULATING tbl_students_transactions_test TABLE ===")
...
for matric_no, level_id in tqdm(student_courses, desc="  Populating tbl_students_transactions_test:"):
    ...
    last_mod_ts = current_time = (datetime.utcnow() + timedelta(hours=1)).strftime('%Y-%m-%d %H:%M:%S')
...
# Fetch matric_no, utme_no, and surname from tbl_students_master_test
...
# === 13. POPULATE tbl_course_registered_test TABLE ===
print("=== POPULATING tbl_course_registered_test TABLE ===")
...
for record in tqdm(course_records, desc="  Populating tbl_course_registered_test:"):
    ...
    date_registered = current_time = (datetime.utcnow() + timedelta(hours=1)).strftime('%Y-%m-%d %H:%M:%S')
...
print("=== POPULATING course_dept_reg_map_test TABLE ===")
cursor.execute("""
    SELECT 
        course_id_fk,
        course_dept_id_fk,
...
current_time = current_time = (datetime.utcnow() + timedelta(hours=1)).strftime('%Y-%m-%d %H:%M:%S')
inserted_rows = 0
updated_rows = 0
...
for row in tqdm(course_dept_data, desc="  Populating course_dept_reg_map_test:"):
    ...
    if key in existing_keys:
        # Perform update for existing key
...
# === 15. Compile all logs into Excel (with all sheets, even empty ones) ===
        ...
        if os.path.exists(csv_file):
            try:
                ...
                if not df.empty:
                    ...
                    # Empty CSV: add note
                    pd.DataFrame({"Info": [f"No records were logged for '{sheet_name}' during processing."]}).to_excel(
                        writer, sheet_name=sheet_name, index=False
                    )
                os.remove(csv_file)
            except Exception as e:
                print(f"  ! Warning: Could not process {csv_file}: {str(e)}")
                # Still create sheet with error note
                pd.DataFrame({"Info": [f"Could not process '{sheet_name}': {str(e)}"]}).to_excel(
                    writer, sheet_name=sheet_name, index=False
                )
        else:
            # Missing CSV: add note
            pd.DataFrame({"Info": [f"No records were logged for '{sheet_name}' during processing."]}).to_excel(
                writer, sheet_name=sheet_name, index=False
            )

print(f"  ✓ Logs compiled into: {output_excel} (all log sheets included, with notes for empty/missing logs)")
