...
import time
import re  # <-- Added for course level extraction
...
# === 4. Connect to MySQL ===
print("Step 2/4: Establishing database connections...")
...
for _, row in df.iterrows():
    ...
    if normalized_code not in course_code_map:
        ...
        new_courses_inserted.append({
            'course_id': new_course_id,  # Added this line to include the ID
...
# if new_courses_inserted and isinstance(new_courses_inserted, list) and len(new_courses_inserted) > 0:
#     log_df = pd.DataFrame(new_courses_inserted)
    
#     if not log_df.empty and not log_df.columns.empty:
#         log_filename = f"newly_inserted_courses_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
#         log_df.to_excel(log_filename, index=False)
#         print(f"✓ {len(new_courses_inserted)} new courses added and logged to '{log_filename}'\\n")
#     else:
#         print("✓ No valid data to write to Excel.\n")
# else:
#     print("✓ No new courses to log.\n")


# === 6. Create students_courses Table ===
# print("Step 4/4: Creating students_courses Table...", end='')
...
# === 10. Insert Course Records into tbl_students_master ===
...
print("\n2/4: Inserting records into tbl_students_transactions_test table...")
...
skipped_user_rows = []
seen_usernames = set()  # <-- Track within-script duplicates
...
for matric_no, utme_no in tqdm(students_master_data, desc="  Progress level:"):
    ...
    seen_usernames.add(matric_no)  # <-- Avoid inserting again
...
# === 5/5: Update course_dept_reg_map_test table ===
print("\n5/5: Updating course_dept_reg_map_test table...")
print("  - Reconnecting to database...", end=' ')
connection = pymysql.connect(
    host='127.0.0.1',
    user='uirms',
...
# Get data from tbl_course_registered_test
cursor.execute("""
    SELECT 
        course_id_fk,
        course_dept_id_fk,
...
if existing_csvs:
    ...
    # Create empty workbook with one sheet if no CSVs exist
    wb = Workbook()
    ws = wb.active
