...
if filtered_skipped_transactions:
    with open('skipped_transactions.csv', 'w', newline='') as f:
        keys = filtered_skipped_transactions[0].keys()
...
dept_id_to_faculty_map = {row[0]: row[2] for row in dept_data}  # id → faculty_id_fk

# Prepare student mapping
...
# Prepare course info mapping
cursor.execute("SELECT _id, code, dept_id_fk, unit FROM tbl_courses")
courses_data = cursor.fetchall()
course_code_map = {row[1].replace(" ", "").lower(): (row[0], row[2], row[3]) for row in courses_data}
...
for record in course_records:
    ...
    # Skip if essentials are missing
    if not all([matric_no, course_id, course_code, level_id, semester_id, course_status, session_id]):
    ...
    # Normalize course code and fetch details
    normalized_course_code = course_code.replace(" ", "").lower()
    course_info = course_code_map.get(normalized_course_code)
    if not course_info:
        skipped_rows.append({'matric_no': matric_no, 'reason': f'Course code "{course_code}" not found in tbl_courses'})
        continue
    course_id_fk_check, course_dept_id_fk, unit_from_courses = course_info

    if int(course_unit) == 0:
        course_unit = unit_from_courses  # use unit from tbl_courses

    # Determine course_faculty_id_fk
    if course_dept and course_dept.strip():
        dept_key = course_dept.strip().lower()
        dept_info = dept_map.get(dept_key)
        if not dept_info:
            skipped_rows.append({'matric_no': matric_no, 'reason': f'Course department "{course_dept}" not found'})
            continue
        _, course_faculty_id_fk = dept_info
    else:
        course_faculty_id_fk = dept_id_to_faculty_map.get(course_dept_id_fk)
        if not course_faculty_id_fk:
            skipped_rows.append({'matric_no': matric_no, 'reason': f'Faculty not found for course_dept_id_fk {course_dept_id_fk}'})
            continue
    ...
    data_tuple = (
        ...
        semester_id, course_status, datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        session_id, session_id, '', '', '', '', '', '', 'filled by script', '', 0, 0, 0, 0,
        semester_id, '', 0, 0, key, 0, 0
    )
    ...
    try:
        ...
        skipped_rows.append({'matric_no': matric_no, 'reason': str(e)})
