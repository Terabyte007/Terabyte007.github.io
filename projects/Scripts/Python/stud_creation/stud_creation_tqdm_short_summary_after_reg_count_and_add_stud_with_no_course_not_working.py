...
print("Step 4/5: Creating students_courses Table...")
...
print("Step 5/5: Processing students_courses records...")
...
for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing student course registrations"):
    ...
    all_matric_numbers.add(matric_no)
...
# === Add students missing from master table ===
print("\n=== STUDENT VERIFICATION PHASE ===")
print("Checking for students who might have been skipped due to missing courses...")
cursor.execute("SELECT matricNo FROM tbl_students_master_test")
existing_matrics = set(row[0] for row in cursor.fetchall())

missing_students = all_matric_numbers - existing_matrics
added_missing = 0

if missing_students:
    print(f"Found {len(missing_students)} students not in master table")
    for matric_no in tqdm(missing_students, desc="Adding missing students"):
        # Find the first row with this matric number
        student_rows = df[df['student_number'] == matric_no]
        if not student_rows.empty:
            student_row = student_rows.iloc[0]
            
            # Extract student details
            full_name = str(student_row.get("full_name")).strip() if student_row.get("full_name") else ""
            name_parts = full_name.split()
            firstname = name_parts[0].capitalize() if len(name_parts) >= 1 else ''
            surname = name_parts[1].upper() if len(name_parts) >= 2 else ''
            middlename = name_parts[2].capitalize() if len(name_parts) >= 3 else ''

            level_raw = str(student_row.get("level")).lower() if student_row.get("level") else ''
            mode_id = 2 if 'de' in level_raw else 1

            sex_raw = str(student_row.get("sex")).strip().lower() if student_row.get("sex") else ''
            gender = 'Male' if sex_raw == 'male' else 'Female' if sex_raw == 'female' else 'NA'

            faculty_raw = str(student_row.get("student_faculty"))[11:].strip() if student_row.get("student_faculty") else ''
            department_raw = str(student_row.get("student_department")).strip().lower() if student_row.get("student_department") else ''

            faculty_id = faculty_map.get(faculty_raw.lower())
            departments_id = department_map.get(department_raw)

            if faculty_id and departments_id:
                insert_student_query = """
                    INSERT INTO tbl_students_master_test (
                        matricNo, surname, firstname, middlename, gender_id, dob, nationality,
                        session_admitted_id_fk, faculty_id_fk, department_id_fk, programme_id_fk,
                ...
                """
                student_data = (
                    matric_no, surname, firstname, middlename, gender, int(session_id_fk),
                    faculty_id, departments_id, mode_id
                )
                cursor.execute(insert_student_query, student_data)
                added_missing += 1
    ...
    connection.commit()
    print(f"  ✓ Added {added_missing} students who were missing from master table")
else:
    print("  ✓ All students already exist in master table")
...
print("  - Loading students_master data for insertion into tbl_students_transactions_test...", end=' ')
...
print("2/3: Creating student_user accounts...")
...
print("3/3: Finalizing tbl_course_registrations...")
print("  - Reconnecting to database...", end=' ')
connection = pymysql.connect(
    host='127.0.0.1',
    user='uirms',
...
for record in tqdm(course_records, desc="Populating tbl_course_registered table"):
...
# Get final count after all inserts
cursor.execute("SELECT COUNT(*) FROM tbl_course_registered_test")
...
print(f"  ! {len(skipped_rows)} course registrations skipped (see skipped_registeration sheet for details)\n")
