...
for _, row in tqdm(df.iterrows(), total=len(df), desc="  Progress level:"):
    ...
    cursor.execute("SELECT _id, department_id_fk FROM tbl_students_master_test WHERE matricNo = %s", (matric_no,))
    ...
    if not master_row:
        ...
        student_master_id, db_dept_id = master_row
        # If department in master is different from Excel, update all
        if db_dept_id != departments_id and departments_id is not None:
            print(f"Updating department for {matric_no} from {db_dept_id} to {departments_id}")
            # 1. Update tbl_students_master_test
            cursor.execute("""
                UPDATE tbl_students_master_test
                SET department_id_fk = %s
                WHERE matricNo = %s
            """, (departments_id, matric_no))
            # 2. Update tbl_course_registered_test for this student & session
            cursor.execute("""
            ...
            # 3. Update course_dept_reg_map_test for this student & session
            cursor.execute("""
            ...
            connection.commit()
    # --- END DEPARTMENT SYNC/UPDATE LOGIC ---
